...
class myYOLO(nn.Layer):
    ...
    def create_grid(self, input_size):
        ...
        grid_y, grid_x = paddle.meshgrid([paddle.arange(hs), paddle.arange(ws)])
        grid_xy = paddle.stack([grid_x, grid_y], axis=-1)


        grid_xy=paddle.reshape(grid_xy,[1,hs*ws,2])
    ...
    def decode_boxes(self, pred):
        ...
        """
        output = paddle.zeros_like(pred)
        pred[:, :, :2] = paddle.nn.functional.sigmoid(pred[:, :, :2]) + self.grid_cell
        pred[:, :, 2:] = paddle.exp(pred[:, :, 2:])
